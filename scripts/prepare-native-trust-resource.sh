...
candidate_path="$temporary_directory/AuroraSignedSeedTrust.bin"
umask 077
install -m 600 "$SOURCE_PATH" "$candidate_path"
...
actual_trust_sha256="$(sha256_file "$candidate_path")"
if [ "$actual_trust_sha256" != "$EXPECTED_TRUST_SHA256" ]; then
    printf 'signed-seed trust file does not match the reviewed release digest\n' >&2
...
(
    ...
    GOTOOLCHAIN="$GO_TOOLCHAIN" \
        ...
        GOFIPS140=off \
        CGO_ENABLED=0 \
        go run ./cmd/auroractl check-native-provisioning-trust "$candidate_path"
)

mkdir -p "$(dirname "$RESOURCE_PATH")"
install -m 600 "$candidate_path" "$RESOURCE_PATH"
printf 'prepared sealed native trust resource sha256=%s\n' "$actual_trust_sha256"
